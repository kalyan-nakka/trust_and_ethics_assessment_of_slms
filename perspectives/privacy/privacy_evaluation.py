...
from perspectives.privacy.sys_prompts import PROMPT_TEMPLATE, PII_INFO, PII_PROMPT_TYPE, PRIVACY_TOPIC

from perspectives.privacy.utils import get_privacy_understanding_messages, load_enron_email_prompts, \
    get_pii_messages_k_shot
from perspectives.privacy.result_agg import calculate_scores
...
def generate_and_save(args, dataset, gt_list, constructor, out_file, batch_size, keynames=['gt']):
    ...
    with TemporaryDirectory(dir="/home/kalyan/cache") as cache_dir_name:
        model = Chat.from_helm(args, cache=cache_dir_name)
        ...
        for batch_idx in range(num_batches):
            ...
            cost, cache = model.do_generation(dataset=batch_data,
    ...
    print("Num of completion tokens: ", completion_tokens)
    return price_cost, prompt_tokens, completion_tokens, num_prompt
...
def main(args):
    ...
    if args.privacy.scenario_name == "pii":
        ...
        for pii in args.privacy.personal_infos:
            ...
            for few_shot_type in args.privacy.prompt_types:
                if few_shot_type not in PII_PROMPT_TYPE:
                    print(f"Skip {few_shot_type} as it is not defined in supported prompt types {PII_PROMPT_TYPE}")
                    continue

                dataset, gt_list = get_pii_messages_k_shot(filename=args.privacy.data_file,
                                                           k=args.privacy.few_shot_num,
                                                           pp=pii,
                                                           fewshot_type=few_shot_type,
                ...
                out_file = os.path.join(
                    ...
                    "{}_{}shot{}_s{}_{}_{}.jsonl".format(
                        pii,
                        args.privacy.few_shot_num,
                        few_shot_type,
                        dataset_size,
                        args.model_config.model,
                        args.privacy.seed).replace("/", "_").replace(" ", ""))
                print(f"Saving to {out_file}")

                price_cost, prompt_tokens, completion_tokens, num_prompt = generate_and_save(args=args,
                                                                                             dataset=dataset,
                                                                                             gt_list=[gt_list],
                ...
                all_completion_tokens[pii] = completion_tokens
        ...
        for topic in args.privacy.privacy_topics:
            ...
            out_file = os.path.join(
                ...
                "{}_t{}_s{}_{}_ques{}_{}.jsonl".format(
                    topic,
                    args.privacy.template,
                    dataset_size,
                    args.model_config.model,
                    args.privacy.question_prompt,
                    args.privacy.seed).replace("/", "_").replace(" ", ""))
            ...
            all_completion_tokens[topic] = completion_tokens
        ...
        for prompt_type in args.privacy.prompt_types:
            ...
            all_completion_tokens[prompt_type] = completion_tokens
    ...
    completion_tokens_over_all_settings = 0
    ...
    for key in all_cost.keys():
        ...
        print("Num of completion tokens: ", all_completion_tokens[key])
        cost_over_all_settings += all_cost[key]
        prompt_tokens_over_all_settings += all_prompt_tokens[key]
        completion_tokens_over_all_settings += all_completion_tokens[key]
